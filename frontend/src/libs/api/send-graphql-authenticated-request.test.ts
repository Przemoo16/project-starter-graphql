...
describe('[sendGraphQLAuthenticatedRequest function]', () => {
  test(`sends original request with the auth header`, async () => {
  ...
  test(`doesn't handle non graphql errors in original request`, async () => {
    let onUnauthorizedCalled = false;
    const onFetch = async () => {
  ...
  test(`doesn't handle non token related errors in original request`, async () => {
    let onUnauthorizedCalled = false;
    const onFetch = async () => ({
      errors: [
        {
  ...
  test(`calls onUnauthorized callback on token related errors in original request`, async () => {
  ...
  test(`calls onInvalidTokens callback on onUnauthorized callback error`, async () => {
    let onFetchCalledTimes = 0;
    let onInvalidTokensCalled = false;
    const onFetch = async () => {
      onFetchCalledTimes += 1;
  ...
  test(`retry request with new token`, async () => {
  ...
  test(`retry request successfully`, async () => {
    let onFetchCalledTimes = 0;
    let onInvalidTokensCalled = false;
    const headersCalled: Array<Record<string, string> | undefined> = [];
    const onFetch = async (
  ...
  test(`doesn't handle non graphql errors in retry request`, async () => {
    let onFetchCalledTimes = 0;
    let onInvalidTokensCalled = false;
    const onFetch = async () => {
      onFetchCalledTimes += 1;
  ...
  test(`doesn't handle non token related errors in retry request`, async () => {
    let onFetchCalledTimes = 0;
    let onInvalidTokensCalled = false;
    const onFetch = async () => {
      onFetchCalledTimes += 1;
  ...
  test(`calls onInvalidTokens callback in retry request`, async () => {
    let onInvalidTokensCalled = false;
    const onFetch = async () => ({
      errors: [
        {
